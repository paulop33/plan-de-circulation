...
function snapshotInitial(feature, userChanges) {
...
export function toggleDirection(feature, state, updateSource, recomputeRoute) {
	ensureDefaults(feature);
	snapshotInitial(feature, state.userChanges);
	...
	applyChange(feature, state, updateSource, recomputeRoute);
}

export function togglePedestrian(feature, state, updateSource, recomputeRoute) {
	ensureDefaults(feature);
	snapshotInitial(feature, state.userChanges);
...
export function toggleModalFilter(feature, state, updateSource, recomputeRoute) {
	ensureDefaults(feature);
	snapshotInitial(feature, state.userChanges);
...
export function toggleBollard(feature, state, updateSource, recomputeRoute) {
	ensureDefaults(feature);
	snapshotInitial(feature, state.userChanges);
...
export function handleSplit(feature, lngLat, state, updateSource) {
	...
	state.userSplits[osmId] = [featureA, featureB];

	state.data.features = state.data.features.filter(f => f.properties.osm_id !== osmId);
	state.data.features.push(featureA, featureB);
	updateSource();
}
